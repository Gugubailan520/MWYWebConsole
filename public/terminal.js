...
let monitorPanelVisible = false;
let monitorInterval = null;
let prevMonitorStats = null;
let reqIdCounter = 0;
const pendingReqs = {};
...
if (!connInfo) {
  ...
  document.title = connInfo.name + ' - 冥雾云Web控制台';
...
function initTerminal() {
  ...
  term = new Terminal({
    ...
    theme: {
      ...
      blue: '#2563eb', brightBlue: '#60a5fa', magenta: '#d63384', brightMagenta: '#e685b5',
...
function startConnection() {
  ...
  ws.onmessage = (event) => {
    ...
    if (data.type === 'status') {
      ...
      if (settings.autoMonitor) { showMonitorPanel(); }
    ...
    } else if (data.type === 'monitor-stats-result') {
      if (!data.error && data.raw) handleMonitorStats(data.raw);
    }
  };
...
function setConnected(status) {
  ...
  if (!status) { sftpReady = false; stopMonitor(); }
}

// ===== Monitor Panel =====
function toggleMonitorPanel() {
  monitorPanelVisible ? hideMonitorPanel() : showMonitorPanel();
}
function showMonitorPanel() {
  monitorPanelVisible = true;
  const mp = document.getElementById('monitorPanel');
  mp.classList.remove('hidden');
  if (window.innerWidth <= 768) mp.classList.add('mobile-open');
...
function hideMonitorPanel() {
  ...
  const mp = document.getElementById('monitorPanel');
  if (window.innerWidth <= 768) mp.classList.remove('mobile-open');
  mp.classList.add('hidden');
  document.getElementById('btnMonitor').classList.remove('active');
  stopMonitor();
  setTimeout(() => { if (fitAddon && term) fitAddon.fit(); }, 280);
}
function startMonitor() {
  ...
  fetchMonitorStats();
  monitorInterval = setInterval(fetchMonitorStats, 2000);
}
function stopMonitor() {
  if (monitorInterval) { clearInterval(monitorInterval); monitorInterval = null; }
  prevMonitorStats = null;
  const body = document.getElementById('mpBody');
  if (body) body.innerHTML = '<div class="mp-offline">连接后开始监控</div>';
}
function fetchMonitorStats() {
  if (!ws || ws.readyState !== WebSocket.OPEN) return;
...
function handleMonitorStats(raw) {
  ...
  let cpuPercent = 0, cpuStats = null;
  if (cpuLine) {
    const parts = cpuLine.trim().split(/\s+/).slice(1).map(Number);
    const idle = (parts[3] || 0) + (parts[4] || 0);
    const total = parts.reduce((a, b) => a + b, 0);
    cpuStats = { idle, total };
    if (prevMonitorStats && prevMonitorStats.cpu) {
      const dTotal = total - prevMonitorStats.cpu.total;
      const dIdle  = idle  - prevMonitorStats.cpu.idle;
  ...
  netdevRaw.split('\n').forEach(l => {
    ...
    if (m && m[1] !== 'lo') { totalRx += parseInt(m[2]); totalTx += parseInt(m[3]); }
  });
  const dt = prevMonitorStats ? (now - prevMonitorStats.time) / 1000 : 0;
  let rxRate = 0, txRate = 0;
  if (prevMonitorStats && prevMonitorStats.net && dt > 0) {
    rxRate = Math.max(0, (totalRx - prevMonitorStats.net.rx) / dt);
  ...
  diskRaw.split('\n').forEach(l => {
    ...
    if (/^(sd[a-z]|hd[a-z]|vd[a-z]|nvme\d+n\d+|xvd[a-z]|mmcblk\d+)$/.test(p[2])) {
      totalRS += parseInt(p[5]) || 0;
      totalWS += parseInt(p[9]) || 0;
    }
  });
  let readRate = 0, writeRate = 0;
  if (prevMonitorStats && prevMonitorStats.disk && dt > 0) {
    readRate  = Math.max(0, (totalRS - prevMonitorStats.disk.rs) * 512 / dt);
...
function renderFileList(items) {
  ...
  const svgFolder = '<svg viewBox="0 0 24 24" fill="none" stroke="#60a5fa" stroke-width="2" stroke-linecap="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>';
